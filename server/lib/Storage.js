...
_.extend(S3storage.prototype, EventEmitter.prototype, {
      create : function() {
          var config= Mono.getInstance().config.s3;
         // var s3 = new AWS.S3();
...
//          var s3bucket = new AWS.S3({Bucket:'Mono-test-00'});
//          var self = this;
//
//         fs.readFile(file.path, function (err, data) {
//              if (err) throw err;
//              var params = {Bucket:'Mono-test-00',Body: data,Key:file.name};
...
//      } ,
//      get : function() {
//          s3bucket.getObject({Bucket:'Mono-test-00',Key:file.name}).
//                      on('httpData', function(chunk) {
//                          wfile.write(chunk);
//                      }).
//                      on('httpDone', function() {
